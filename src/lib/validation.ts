...
export function validatePlayerName(name: string): ValidationResult {
  ...
  if (trimmedName.length > GAME_CONFIG.MAX_PLAYER_NAME_LENGTH) {
    return { isValid: false, error: `Le nom ne peut pas dépasser ${GAME_CONFIG.MAX_PLAYER_NAME_LENGTH} caractères` };
  }
...
export function validateRoomCode(code: string): ValidationResult {
  ...
  if (trimmedCode.length !== GAME_CONFIG.ROOM_CODE_LENGTH) {
    return { isValid: false, error: `Le code de la salle doit contenir exactement ${GAME_CONFIG.ROOM_CODE_LENGTH} caractères` };
  }

  // Check if code contains only valid characters
  const validChars = /^[A-Z0-9]+$/;
  if (!validChars.test(trimmedCode)) {
    return { isValid: false, error: "Le code de la salle ne peut contenir que des lettres majuscules et des chiffres" };
  }
...
export function validateSharedWord(word: string): ValidationResult {
  ...
  if (trimmedWord.length < 2) {
    return { isValid: false, error: "Le mot doit contenir au moins 2 caractères" };
  }

  if (trimmedWord.length > GAME_CONFIG.MAX_WORD_LENGTH) {
    return { isValid: false, error: `Le mot ne peut pas dépasser ${GAME_CONFIG.MAX_WORD_LENGTH} caractères` };
  }

  // Check for invalid characters
  const invalidChars = /[<>\"'&]/;
  if (invalidChars.test(trimmedWord)) {
    return { isValid: false, error: "Le mot contient des caractères non autorisés" };
  }
...
export function validateGameConfig(
  ...
  if (totalSpecialRoles >= playerCount) {
    return {
      isValid: false,
      error: "Il faut au moins 1 joueur civil. Réduisez le nombre d'undercovers ou désactivez Mr. White."
    };
  }
 ...
 */
export function sanitizeInput(input: string, maxLength: number = GAME_CONFIG.MAX_PLAYER_NAME_LENGTH): string {
  return input
    .trim()
    .slice(0, maxLength)
...
export function validateSessionId(sessionId: string): ValidationResult {
  ...
  if (!validChars.test(sessionId)) {
    return { isValid: false, error: "Session ID contient des caractères invalides" };
  }
