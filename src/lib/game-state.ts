...
export class GameStateManager {
   ...
   */
  static async transitionToNextPhase(ctx: any, roomId: string, currentState: string) {
  ...
  static async processVotingResults(ctx: any, roomId: string) {
    ...
    const gameResult = GameStateService.checkGameEnd(alivePlayers, room.currentRound, room.maxRounds);
  ...
  static async validateAndFixGameState(ctx: any, roomId: string) {
    ...
    if (room.gameState === "discussion" && room.playerOrder && room.currentPlayerIndex !== undefined) {
    ...
    if (room.gameState === "voting") {
      const allAlivePlayersVoted = GameFlowHelpers.allPlayersCompletedAction(alivePlayers, 'voted');
